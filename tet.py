    ...
    def on_draw(self):
        ...
        for z in [300, 200]:
            ...
            self.background.draw_transformed(
                ...
                Matrix3x3().scale(scale, scale).translate(-self.camera_x * translate, 0))
...
class GameView(arcade.View):
    ...
    def __init__(self):
        ...
        self.tetris_shapes = [
            ...
            [YELLOW, [[1,2,5,6]]],
            [AQUA, [[1,4,5,6], [1,4,5,9], [4,5,6,9], [1,5,6,9]]],
            [BLUE, [[1,2,5,9], [0,4,5,6], [1,5,8,9], [4,5,6,10]]],
            [ORANGE, [[0,1,5,9], [4,5,6,8], [1,5,9,10], [2,4,5,6]]],
        ...
        # Load sounds
        self.bottom_sound = arcade.load_sound("sounds/se_maoudamashii_se_sound16.wav")
        self.delete_sound = arcade.load_sound("sounds/se_maoudamashii_battle07.wav")
        self.attacked_sound = arcade.load_sound("sounds/se_maoudamashii_system26.wav")
        self.levelup_sound = arcade.load_sound("sounds/se_maoudamashii_system29.wav")
        self.gameover_sound = arcade.load_sound("sounds/se_maoudamashii_retro30.wav")
    ...
    def on_show(self):
        ...
        if self.window.game_mode == 0:
            ...
            self.background = arcade.load_texture("images/mandala-1094811_1280.jpg")
        else:
            # image for 2-player mode
            self.background = arcade.load_texture("images/fractal-1832617_1280.jpg")
    ...
    def on_draw(self):
        ...
        if self.window.debug == True:
            ...
            arcade.draw_text(f"Loop time: {loop_time * 1000:.3f} (Update time: {self.update_time * 1000:.3f}, Draw time: {draw_time * 1000:.3f})", 60, 0, arcade.color.WHITE, 14)
    ...
    def on_update(self, delta_time):
        ...
        for player in self.players:
            ...
            elif player.delete_animation == True:
                # Delete animation
            ...
            if player.player_moved:
                ...
                for pos in self.tetris_shapes[player.shape][1][player.shape_cnt]:
                    x = player.x + pos % 4
                    y = player.y - pos // 4
                ...
                for y in range(0, PLHEIGHT):
                    for x in range(0, PLWIDTH):
                        if player.game_area[y][x] != 0:
                            block = player.display_block(player.game_area[y][x], x, y)
    ...
    def on_key_press(self, key, modifiers):
        ...
        if key == arcade.key.UP or key == arcade.key.W:
            for player in self.players:
                if player.player_num != 2 and player.delete_animation == False:
...
class Player():
    ...
    def display_block(self, color, x, y):
        ...
        if self.delete_animation == True and y in self.delete_animation_lines:
            ...
            if self.delete_animation_index > 7:
                ...
                if self.game_view.window.game_mode == 1:
                    for player in self.game_view.players:
                        if self != player:
                            player.damage_lines = len(self.delete_animation_lines) - 1
    ...
    def shape_move(self):
        ...
        if self.up_pressed:
            ...
            self.shape_cnt += 1
            if self.shape_cnt >= len(self.game_view.tetris_shapes[self.shape][1]):
                self.shape_cnt = 0
    ...
    def shape_fall(self):
        ...
        if self.fall_counter <= 0 or self.fall_flag == True:
            self.fall_counter = self.game_view.fall_counter_init[self.level]
            self.player_moved = True
            if self.generate_tetris == True:
                ...
                if not self.can_move():
                    ...
                    if self.delete_animation == False:
                        # No delete line
                        self.generate_tetris = True
